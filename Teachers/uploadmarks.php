...
<html lang="en">
<head>
    <title>Teachers Upload Marks | SIMS</title>
    ...
    <div class="row">
      ...
      <main class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
        <div
          ...
          <h4 class="h3">Upload student marks</h4>
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    if ($result_students) {
                        ...
                        if (mysqli_num_rows($result_students) > 0) {
                            ...
                            <div class="container">
                            <form name="uploadmarks.php" method="POST" action="../uploadmarks.php">
                            <div class="row">
                                <div class="col-md-4">
                                <h5 class="mt-0 mb-1">Enter Marks</h5>
                                <input type="number" class="form-control mt-0" name="marks" placeholder="Enter Marks">
                                </div>
                                '; // Use proper concatenation

                            $sql_subject = "SELECT * FROM subjects ";
                            $result_subjects = mysqli_query($db, $sql_subject);

                            if ($result_subjects) {
                                if (mysqli_num_rows($result_subjects) > 0) {
                                    echo '
                                    <div class="col-md-4">
                                        <h5 class="mt-0 mb-1">Select Subject</h5>
                                        <select name="subject" class="form-control" required>
                                            <option value="">Select Subject</option>';
                                    while ($row = mysqli_fetch_assoc($result_subjects)) {
                                        $subjectName = $row['subject_name'];
                                        $subjeid = $row['id'];
                                        echo "<option value='$subjeid'>$subjectName</option>";
                                    }
                                    echo '</select></div>';
                                }
                            }
